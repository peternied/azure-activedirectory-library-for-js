//----------------------------------------------------------------------
// [TODO]: The version should be updated to something that isn't related to my github account, v1.0.13-ServiceDesk
// AdalJS v1.0.13-peternied.5
            ...
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout', '$injector', function ($rootScope, $window, $q, $location, $timeout, $injector) {
                ...
                var locationChangeHandler = function (event, newUrl, oldUrl) {
                    ...
                    if (_adal.isCallback(hash)) {
                        ...
                        if (!requestInfo.valid) {
                            _adal.warn("Invalid request info");
                        }

                        _adal.saveTokenFromHash(requestInfo);

                        if (requestInfo.stateMatch) {
                            _adal.verbose("Return to callback if it is sent from iframe");
                            if (requestInfo.requestType === _adal.REQUEST_TYPE.RENEW_TOKEN) {
                                ...
                                _adal.verbose("Since this is a token renewal request in iFrame, we don't need to proceed with the location change.")
                                event.preventDefault();

                                _adal.verbose("Call within the same context without full page redirect keeps the callback");
                                if (callback && typeof callback === 'function') {
                                    _adal.verbose("id_token or access_token can be renewed");
                                    if (requestInfo.parameters['access_token']) {
                                        ...
                                        _adal.verbose("$broadcast, adal:tokenRenewalSuccess access_token");
                                        // [TODO] These events, or something like them should be promoted into the ADAL codebase
                                        $rootScope.$broadcast('adal:tokenRenewalSuccess', 'access_token');
                                        return;
                                        ...
                                        _adal.verbose("$broadcast, adal:tokenRenewalSuccess id_token");
                                        // [TODO] These events, or something like them should be promoted into the ADAL codebase
                                        $rootScope.$broadcast('adal:tokenRenewalSuccess', 'id_token');
                                        return;
                                        ...
                                        _adal.verbose("$broadcast, adal:tokenRenewalFailure " + _adal._getItem(_adal.CONSTANTS.STORAGE.ERROR_DESCRIPTION) + " " + _adal._getItem(_adal.CONSTANTS.STORAGE.ERROR));
                                        // [TODO] These events, or something like them should be promoted into the ADAL codebase
                                        $rootScope.$broadcast('adal:tokenRenewalFailure', _adal._getItem(_adal.CONSTANTS.STORAGE.ERROR_DESCRIPTION), _adal._getItem(_adal.CONSTANTS.STORAGE.ERROR));
                                        return;
                                    }
                                }
                            } else if (requestInfo.requestType === _adal.REQUEST_TYPE.LOGIN) {
                                _adal.verbose("normal full login redirect happened on the page");
                                updateDataFromCache(_adal.config.loginResource);
                                if (_oauthData.userName) {
                                    $timeout(function () {
                                        _adal.verbose("id_token is added as token for the app");
                                        updateDataFromCache(_adal.config.loginResource);
                                        $rootScope.userInfo = _oauthData;
                                    }, 1);
                                    _adal.verbose("$broadcast, adal:loginSuccess from full normal redirect");
                                    $rootScope.$broadcast('adal:loginSuccess', _adal._getItem(_adal.CONSTANTS.STORAGE.IDTOKEN));
                                } else {
                                    _adal.verbose("$broadcast, adal:loginFailure: " + _adal._getItem(_adal.CONSTANTS.STORAGE.ERROR_DESCRIPTION) + ", " + _adal._getItem(_adal.CONSTANTS.STORAGE.ERROR));
                                ...
                                if (!_adal.popUp) {
                                    _adal.verbose("redirect to login start page in non-popup mode");
                                    ...
                                    if (typeof loginStartPage !== 'undefined' && loginStartPage && loginStartPage.length !== 0) {
                                        _adal.verbose("prevent the current location change and redirect the user back to the login start page");
                                        _adal.verbose('Redirecting to start page: ' + loginStartPage);
                                        $window.location = loginStartPage;
                                    }
                                }
                                else {
                                    _adal.verbose("In popup mode, so preventing location change for login");
                                    event.preventDefault();
                                }
                            }
                        }
                        else {
                            _adal.verbose("state did not match, broadcast an error");
                            _adal.verbose("$broadcast, adal:stateMismatch: " + _adal._getItem(_adal.CONSTANTS.STORAGE.ERROR_DESCRIPTION) + ", " + _adal._getItem(_adal.CONSTANTS.STORAGE.ERROR));
                        ...
                        if (!_oauthData.isAuthenticated && _oauthData.userName && !_adal._renewActive) {
                            _adal.verbose("id_token is expired or not present");
                            ...
                            self.acquireToken(_adal.config.loginResource).then(function (token) {
                                ...
                                _adal.verbose("$broadcast, adal:loginFailure, " + errorParts[0] + ", " + errorParts[1]);
                ...
                var loginHandler = function () {
                    ...
                    if (_adal.config && _adal.config.localLoginUrl) {
                        ...
                        _adal.verbose("$broadcast, adal:loginRedirect");
                ...
                return {
                    ...
                    acquireToken: function (resource) {
                        _adal.verbose("$broadcast, adal:acquireTokenStarted");
                        // [TODO] These events, or something like them should be promoted into the ADAL codebase
                        $rootScope.$broadcast('adal:acquireTokenStarted');
                        ...
                        _adal.acquireToken(resource, function (errorDesc, tokenOut, error) {
                            ...
                            if (error) {
                                _adal.verbose("$broadcast, adal:acquireTokenFailure, " + errorDesc + ", " + error);
                                ...
                                _adal.verbose("$broadcast, adal:acquireTokenSuccess");
                    ...
                    if (config) {
                        ...
                        if (tokenStored) {
                            ...
                            if (authService.loginInProgress()) {
                                authService.verbose("Cancel request if login is starting");
                                ...
                                authService.verbose("delayed request to return after iframe completes");
                                var delayedRequest = $q.defer();
