...
var AuthenticationContext = (function () {
    ...
    AuthenticationContext.prototype.login = function () {
        ...
        // [TODO] This should be removed at this point, as this fork ignores state mismatch
        this.verbose('Expected state: ' + expectedState + ' startPage:' + window.location);
        // [TODO] This should be removed at this point, as this fork ignores state mismatch
        if (this._getItem(this.CONSTANTS.STORAGE.STATE_LOGIN)) {
            this.verbose("Overriding previously expected state: " + this._getItem(this.CONSTANTS.STORAGE.STATE_LOGIN))
        }
    ...
    */
    ...
    AuthenticationContext.prototype._addHintParameters = function (urlNavigate) {
        // [Observation]: This code is was out for a pull request and ignored, https://github.com/AzureAD/azure-activedirectory-library-for-js/pull/502,
        //  we need this change as this was the original reason for this fork to be created
    ...
    AuthenticationContext.prototype.getRequestInfo = function (hash) {
        ...
        // [Observation]: This fixed a ton of the login related failure we saw in production, which at this point I've chalked up to local stoarge
        //  being inconstant
        if (!requestInfo.stateMatch) {
            requestInfo.requestType = this.REQUEST_TYPE.LOGIN;
            requestInfo.stateMatch = true;
            this.warn('Ignoring invalid state match, response state was ' + stateResponse);
        }
    ...
    AuthenticationContext.prototype.saveTokenFromHash = function (requestInfo) {
        this.info('State matches:' + requestInfo.stateMatch + '; Request type:' + requestInfo.requestType);
        this._saveItem(this.CONSTANTS.STORAGE.ERROR, '');
        this._saveItem(this.CONSTANTS.STORAGE.ERROR_DESCRIPTION, '');
        ...
        if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
            ...
            if (requestInfo.stateMatch) {
                ...
                if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ID_TOKEN)) {
                    ...
                    if (this._user && this._user.profile) {
                        if (this._user.profile.nonce !== this._getItem(this.CONSTANTS.STORAGE.NONCE_IDTOKEN)) {
                            this.warn('Ignoring invalid nonce match, nonce in memory: ' + this._idTokenNonce + ', in storage: ' + this._getItem(this.CONSTANTS.STORAGE.NONCE_IDTOKEN) + ', from id_token: ' + this._user.profile.nonce);
                        }
                ...
                this._saveItem(this.CONSTANTS.STORAGE.ERROR_DESCRIPTION, 'Invalid_state. state: ' + requestInfo.stateResponse + ', expected state: ' + this._getItem(this.CONSTANTS.STORAGE.STATE_LOGIN));
            }
        }
    ...
    AuthenticationContext.prototype._saveItem = function (key, obj) {
        // [TODO]: All of this code should be removed, this logging never found anything useful and just made alot of chatter
        if (this.config && this.config.cacheLocation && this.config.cacheLocation === 'localStorage') {
            ...
            this.info("Starting to write key " + key + " with value '" + obj + "'");
            localStorage.setItem(key, obj);
            var writtenObj = this._getItem(key);
            this.info("Written key " + key + " value is '" + writtenObj + "'");
            if (obj != writtenObj) {
                this.warn("Found mismatched item for key " + key + " where original was '" + obj + "' and written value is '" + writtenObj + "'");
            }
    ...
    // [TODO]: This was added to lower stress on local storage, but it didn't have any detectable difference in the failure rate
    /**
     * Returns true if browser supports localStorage, false otherwise.
     * @ignore
     */
    var supportsLocalStorageResult = undefined;
    AuthenticationContext.prototype._supportsLocalStorage = function () {
        if (supportsLocalStorageResult !== undefined) {
        ...
        try {
            ...
            return supportsLocalStorageResult = supportsLocalStorage;
        } catch (e) {
            return supportsLocalStorageResult = false;
        }
    };
     ...
     * @ignore
     */
    var _supportsSessionStorageResult = undefined;
    AuthenticationContext.prototype._supportsSessionStorage = function () {
        if (_supportsSessionStorageResult !== undefined) {
            return _supportsSessionStorageResult;
        }
        try {
            ...
            return _supportsSessionStorageResult = supportsSessionStorage;
        } catch (e) {
            return _supportsSessionStorageResult = false;
        }
    };
